...
**Demo video:** [https://youtu.be/eitsBxL4dQU](https://youtu.be/eitsBxL4dQU)
...
- Full capture → AI extraction → hash/commitment → TEE certification → Coston2 anchoring pipeline
- Claim investigation pipeline: Tavily live search + Groq verdict classification, with prompt-injection-resistant untrusted-content delimiters
- Bring-your-own-wallet anchoring flow (prepare/confirm), so a logged-in user signs with their own key instead of the shared submitter
- Public discovery network: publish/fork claims, canonical-claim dedup via trigram similarity, `/api/discover`, domain trust signal
- Community evidence contributions kept in a separate table from AI-retrieved sources, deliberately with no truth-voting mechanic

## Deployment Status

Currently deployed on **Coston2**.

## Roadmap / Next Steps

- Flare Mainnet deployment once BYOW anchoring is stress-tested on Coston2
- Public API for the domain trust signal
- Weight per-source star ratings into the verdict confidence score

## Traction / Distribution

[fill in if real — pilot users, feedback, early usage signal; leave out rather than pad it]
